...
func process(filename string) {
	...
	unusedSpace := totalDiskSpace - filesystem["/"]
	// log.Printf("unused disk space: %v", unusedSpace)
	neededSpace := requiredSpace - unusedSpace
	// log.Printf("needed disk space: %v", neededSpace)

	allSizes := maps.Values(filesystem)
	bigDirs := Filter(allSizes, func(v int64) bool { return v >= neededSpace })
	sort.Slice(bigDirs, func(a, b int) bool { return bigDirs[a] < bigDirs[b] })
	solution := bigDirs[0]
